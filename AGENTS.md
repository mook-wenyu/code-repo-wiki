...
- `.repo-wiki/wiki/{lang}/` — 模块页（每模块一份，含职责/实体/依赖/使用示例）
- `.repo-wiki/wiki/{lang}/api.md` — API 参考（按模块分组）
- `.repo-wiki/wiki/{lang}/architecture.md` — 架构概览
- `.repo-wiki/wiki/{lang}/overview.md` — 项目概览（自底向上合成）
- `.repo-wiki/cards/{lang}/` — Knowledge Card（AI 代理的结构化摘要，JSON 元数据+Markdown）
- `.repo-wiki/assets/diagrams/` — Mermaid 调用图/依赖图

## AI 代理使用指引

1. 先读 `.repo-wiki/wiki/{lang}/overview.md` 与 `architecture.md` 建立全局认知，
   再按需深入模块页。
...
5. 知识沉淀：`repo-wiki note "<记录>"` 追加到 `.repo-wiki/wiki/{lang}/_log.md`。
