...
impl RustProcessor {
    ...
    fn parse_use_stmt(text: &str, line: usize, out: &mut Vec<ImportStmt>) {
        ...
        if let Some(pos) = inner.find(" as ") {
            out.push(ImportStmt {
                source: inner[..pos].trim().to_string(),
                alias: Some(inner[pos + 4..].trim().to_string()),
                line,
            });
        } else {
            out.push(ImportStmt {
    ...
    fn collect_doc_comments(source: &str) -> Vec<(usize, String)> {
        ...
        for (i, line) in source.lines().enumerate() {
            ...
            if let Some(d) = t.strip_prefix("///") {
                docs.push((i + 1, d.trim_start().to_string()));
    ...
    fn associate_doc(entity: &mut Entity, docs: &[(usize, String)]) {
        ...
        for &(line, ref text) in docs {
            if line >= entity.line_start {
                break;
            }
            if entity.line_start - line <= 3
                && (collected.is_empty()
                    || line == collected.last().map(|&(l, _)| l).unwrap_or(0) + 1)
            {
                collected.push((line, text.clone()));
            } else if line < entity.line_start - 3
                || (!collected.is_empty()
                    && line != collected.last().map(|&(l, _)| l).unwrap_or(0) + 1)
            {
                collected.clear();
            ...
            if let Some(rest) = t.strip_prefix("use ") {
                ...
                imports.push(ImportStmt {
                    source: inner
                        ...
                        .trim()
                        .to_string(),
                    alias: inner.split(" as ").nth(1).map(|s| s.trim().to_string()),
                    line: line_no,
                });
                continue;
            }
...
"#;
        ...
        assert_eq!(
            statics,
            vec!["VERSION"],
            "static_item 应产出 kind=static 实体"
        );
