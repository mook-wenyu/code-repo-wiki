...
impl RustProcessor {
    ...
    fn fallback(source: &str) -> (Vec<Entity>, Vec<ImportStmt>) {
        ...
        for (i, line) in source.lines().enumerate() {
            ...
            if let Some(name) = core.strip_prefix("struct ").and_then(|s| n(s, &['{', ' ', '<', ';'])) { entities.push(Entity { name, kind: "struct".into(), line_start: line_no, line_end: line_no, doc_comment: None, signature: None }); }
            else if let Some(name) = core.strip_prefix("fn ").and_then(|s| n(s, &['(', ' ', '<'])) { entities.push(Entity { name, kind: "function".into(), line_start: line_no, line_end: line_no, doc_comment: None, signature: Some(t.to_string()) }); }
            else if let Some(name) = core.strip_prefix("trait ").and_then(|s| n(s, &['{', ' ', '<'])) { entities.push(Entity { name, kind: "trait".into(), line_start: line_no, line_end: line_no, doc_comment: None, signature: None }); }
            else if let Some(name) = core.strip_prefix("enum ").and_then(|s| n(s, &['{', ' ', '<'])) { entities.push(Entity { name, kind: "enum".into(), line_start: line_no, line_end: line_no, doc_comment: None, signature: None }); }
            else if let Some(name) = core.strip_prefix("mod ").and_then(|s| s.strip_suffix(';')).map(|s| s.trim().to_string()).filter(|s| !s.is_empty()) { entities.push(Entity { name, kind: "mod".into(), line_start: line_no, line_end: line_no, doc_comment: None, signature: None }); }
        }
        (entities, imports)
    }
}
...
impl LanguageProcessor for RustProcessor {
    ...
    fn parse(&self, source: &str, path: &Path) -> Result<FileInsight> {
        if source.is_empty() {
            return Ok(FileInsight { path: path.to_path_buf(), language: "Rust".into(), entities: vec![], imports: vec![], doc_comments: vec![] });
        }
        let (entities, imports) = Self::extract(source);
        if entities.is_empty() && !source.trim().is_empty() {
            return Ok(FileInsight { path: path.to_path_buf(), language: "Rust".into(), entities: Self::fallback(source).0, imports: Self::fallback(source).1, doc_comments: vec![] });
        }
        Ok(FileInsight { path: path.to_path_buf(), language: "Rust".into(), entities, imports, doc_comments: vec![] })
    }
}
