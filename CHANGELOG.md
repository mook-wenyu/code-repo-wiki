...
- **知识卡片三类化（对齐 Qoder：架构文档 / 代码规约 Spec / 技术栈）**：`KnowledgeCard` 新增 `card_kind` 标签（serde default 兼容旧卡旧快照）——模块卡（= 架构文档类，既有能力打标）；**Spec 卡**（`cards/{lang}/project/spec.md`，项目级一张）：输入 = `wiki_plan.yaml knowledgecard.notes` + 仓库规约文件（AGENTS.md/.editorconfig/rustfmt.toml/clippy.toml/CONTRIBUTING.md/docs/glossary.md），LLM 提炼结构化规约（分类 + 条目 + 来源文件锚定防幻觉），**无输入不生成**（不编造）；**技术栈卡**（`cards/{lang}/project/tech-stack.md`，项目级一张）：依赖清单**确定性解析零 LLM**（Cargo.toml/Cargo.lock/package.json/pyproject.toml/requirements.txt/go.mod，lock 为权威版本源，不输出传递依赖；XML 形态清单不支持——限制项已注明）；项目卡与模块卡路径子目录隔离（`project/`）；渲染/`_index.json`（kind 字段）/llms.txt/MCP `wiki_read_card`/`card generate spec|tech-stack` 命令全链路接入
- **项目卡增量闭环**：清单/规约文件纳入指纹基线（`from_insights`）与变更检出（非 git 裸 update 三态：变更→重生成、未变→no-op、删除→差集清理；git 仓库走 GitDiff 天然覆盖）；权威输入集合 `model::PROJECT_CARD_INPUT_FILES`（generate/incremental 共同依赖的底层，避免模块循环依赖）；快照回填保证未变更时项目卡不丢、差集清理不误删
- **`repowiki.template` 新增 `"product_requirement"`**：模块页按产品需求格式输出（需求背景 / 目标用户与场景 / 功能需求·用户故事 / 非功能需求 / 验收标准 / 边界与例外）——Qoder 官方仅描述"按产品需求格式输出"无更细规范（help.aliyun.com/zh/lingma/repo-wiki），格式为本项目合理默认，README 已注明
