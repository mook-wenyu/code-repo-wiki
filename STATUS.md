...
- 提交：c45b318（feat scanner fixtures）→ 69619d2（refactor tests 拆分）→ 9920b26（docs 注释路径）→ b102fa2（feat analysis 跨域约束）→ 97439b5（feat bench gamma_scan）→ bef617a（test large_fixture 适配）→ 4d933da（style fmt）→ ae98195（fix lint 系列名）→ 4e416f2（docs status 初版）→ **6e51a1e（feat analysis 社区大小上限拆分）** → **4e17fd6（fix generate index 链接校正）**
- **大小拆分补充（用户拍板"社区大小上限后处理拆分"）**：`split_oversized_communities`（>20 文件社区按目录键拆子簇，保留 ≤20 跨目录合并能力）——γ 实证最大社区 **73→11 文件**、模块数 20→30；单测 2 个（纯函数直测 + detect 上限不变量）
- **index 链接校正（复验实证修复）**：`canonicalize_module_links`——LLM 输出 index 把模块名连字符误改下划线（plugin-template → plugin_template 断链）+ 编造模块（tests::ingest）→ 按链接文本模块名反查权威集重写文件名、编造降级纯文本；单测 1 个
- **最终复验（30 模块/32 页）**：generate --force exit 0（282s）→ update 补页 2 轮（search::mod 系统性失败 2 次后重试成功，失败模块 0）→ **lint：broken 0 / stale 0 / orphan 0**，残余 **entity-coverage 3 条**（src.md 声称 install_options、tests_incremental 声称 2 个 watch 测试函数——**真实 LLM 编造**（带伪引用，源码查证不存在），防幻觉闸正确拦截；生成侧实体声称校验（反馈重试）列为独立改进项，非 U1 范围）
- 已知风险（诚实自曝）：①**src::search::mod 页全量生成系统性失败**（LLM 声称子文件级依赖被依赖校验拦截——校验正确，update 补偿必成功，全量后需 1-2 轮 update；根治需实体声称校验/提示词增强，独立改进项）；②entity-coverage 3 条残余（真实 LLM 编造，门禁正确拦截；生成侧增强可减少）；③src/search 拆 3 页 + src::config::plugin-template 页（连字符模块名，index 链接已校正）——有边/孤立混合划分现象，语义完整；④语义索引降级（embed key 未注入 + MCP 进程持 db 句柄 os error 32，U16 既有）；⑤tests 功能域页 entity-ownership 23 条 warning（A8 宽容）；⑥PATH 旧版 exe 与当前代码不一致（复验须 cargo run）
- 下次最该做的事：**生成侧实体声称校验**（模块页声称实体 ∉ 模块清单 → 反馈重试，系统性降低 LLM 编造——覆盖 search::mod 依赖虚构与 entity-coverage 残余两问题）；可选：bench 基线重录、U16 mcp 句柄、`.codegraph/` 入 .gitignore（U20）
